...
const run = (msg: Message, args: Array<string>) => {
    ...
    if (args[0] && msg.mentions.members?.first() && args[0] == `<@!${msg.mentions.members?.first()?.id}>`) {    // Verifies that the first argument is a mentionned member
        ...
        memberToBan.ban({ reason: reason }).then(banned => {                                                    // If member banned successfully, sends confirmation
...
// Export ban command
export const banCommand = new Command(name, description, type, howto, permission, run);